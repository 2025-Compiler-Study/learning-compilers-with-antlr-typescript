...
function collectLines(node: AstNode, prefix: string, isLast: boolean, out: string[]): void {
  ...
  const recurse = (kids: AstNode[]) => kids.forEach((k, i) => collectLines(k, next, i === kids.length - 1, out));

  if (node instanceof Program) {
    push("Program");
    recurse(node.functions);
  } else if (node instanceof FuncDef) {
    push(`FuncDef ${node.returnType} ${node.name}`);
    recurse([...node.params, node.body]);
    ...
    push("ReturnStmt");
    recurse(node.value ? [node.value] : []);
  } else if (node instanceof BinaryExpr) {
    push(`BinaryExpr(${node.op})`);
    recurse([node.left, node.right]);
  } else if (node instanceof CallExpr) {
    push(`CallExpr(${node.callee})`);
    recurse(node.args ?? []);
...
export function printAst(program: Program): void {
  ...
  program.functions.forEach((fn, i) => collectLines(fn, "", i === program.functions.length - 1, lines));
