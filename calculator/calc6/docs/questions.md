...
**결론: 3가지 다 AstBuilder.** "AstBuilder가 실행 관점까지 책임져야 하나"라는 우려에 대한 답은 —
...
### 더 깊은 질문 — `AstBuilder`가 두 가지 역할을 겸하고 있는 것 아닌가?
...
- **A. 지금처럼 유지 (채택)** — `AstBuilder`가 변환+시맨틱 분석을 계속 겸하고, main 관련 체크도
  기존 `UndeclaredVariable`/`RedeclaredIdentifier`와 같은 자리에 추가.
- **B. 진짜로 분리** — `SemanticAnalyzer` 같은 새 클래스를 만들어 기존 시맨틱 체크(함수 등록 포함)를
  전부 옮기고 `AstBuilder`는 순수 변환만 담당. 구조적으로는 더 깨끗하지만, 막 리뷰를 마치고 merge한
  기존 코드를 통째로 옮기는 큰 리팩터링이라 지금 범위(main 검증 3개 추가)보다 훨씬 커진다.
