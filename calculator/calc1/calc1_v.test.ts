...
describe("Calc1Visitor", () => {
  it("단순 변수 할당", () => {
    const variables = calculateWithVisitor("a = 1;");
    expect(variables.get("a")).toBe(1);
  });

  it("복잡한 수식 계산", () => {
    const variables = calculateWithVisitor("a = 1 + 2 * 3;");
    expect(variables.get("a")).toBe(7);
  });

  it("여러 변수 할당", () => {
    const variables = calculateWithVisitor("a = 1; b = 2;");
    expect(variables.get("a")).toBe(1);
    expect(variables.get("b")).toBe(2);
  });

  it("변수 참조", () => {
    const variables = calculateWithVisitor("a = 1; b = a + 2;");
    expect(variables.get("a")).toBe(1);
    expect(variables.get("b")).toBe(3);
  });

  it("여러 변수 참조", () => {
    const variables = calculateWithVisitor("a = 1; b = 2; c = a + b;");
    expect(variables.get("a")).toBe(1);
    expect(variables.get("b")).toBe(2);
  ...
  it("선언되지 않은 변수는 0으로 처리", () => {
    const variables = calculateWithVisitor("a = b + 1;");
    expect(variables.get("a")).toBe(1);
  });

  it("다양한 변수명 사용", () => {
    const variables = calculateWithVisitor("aa = 1; BB = aa + 2;");
